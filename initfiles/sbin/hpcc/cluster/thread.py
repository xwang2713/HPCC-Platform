...
class ThreadWithQueue(threading.Thread):
    ...
    def __init__(self, tid, queue):
        ...
        self.keepAlive = True
        self._id = tid
        #self.logger = logging.getLogger("hpcc.cluster.ThreadWithQueue." + str(tid) )
    ...
    def run(self):
        while self.keepAlive:
            try:
                # block is false, timeou is 1 second. Ignore Queue.Empty exception
                # thread is controlled by keepAlive
                items = self.queue.get(False, 1)
                func = items[0]
                args = items[1:]
                #self.logger.info("run " + func + " " + "".join(args))
                func(*args)

                self.queue.task_done()
                #self.logger.info(func + "  done")
