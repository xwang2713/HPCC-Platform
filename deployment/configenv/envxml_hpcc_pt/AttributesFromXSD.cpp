...
AttributesFromXSD::AttributesFromXSD(EnvHelper* peh):m_eh(peh), m_pCompTree(NULL), 
  m_pSchemaRoot(NULL),m_numAttrs(0),m_allSubTypes(true),m_genOptional(true)
{
    m_pEnv.set(peh->getEnvTree());
}


void AttributesFromXSD::CreateAttributeFromSchema(IPropertyTree& attr,
...
void AttributesFromXSD::AddAttributeFromSchema(IPropertyTree& schemaNode, 
...
void AttributesFromXSD::AddAttributesFromSchema(IPropertyTree* pSchema,
       StringBuffer& compName, const char* childElementName)
{
  if (pSchema)
  {
...
void AttributesFromXSD::ProcessElementSchemaNode(IPropertyTree* pElement, 
...
void AttributesFromXSD::ProcessComplexTypeSchemaNode(IPropertyTree* schemaNode, 
       IPropertyTree* pParentElement, StringBuffer& sbCompName)
{
  if (schemaNode)
  {
...
bool AttributesFromXSD::generateHeaders()
{
...
void AttributesFromXSD::setCompTree(IPropertyTree* pTree, IPropertyTree* schemaTree, bool allSubTypes)
{
  m_pCompTree = pTree;
  m_allSubTypes = allSubTypes;
  m_pSchemaRoot.setown(schemaTree);
}

void AttributesFromXSD::getValueForTypeInXSD(IPropertyTree& attr,
       StringBuffer compName, StringBuffer& wizDefVal)
{
  ...
  if (attr.hasProp("./xs:annotation/xs:appinfo/autogendefaultvalue"))
  {
    ...
    else
    {
      ...
      {
        if (((SWProcess *)m_eh->getEnvSWComp(m_compName.str()))->getNumOfNodes() > 1)
        {
  ...
  else if (!strcmp(type,"computerType"))
  {
    if (m_wizFlag)
    {
      ...
      {
        StringArray ipArray;
        ((SWProcess *)m_eh->getEnvSWComp(m_compName.str()))->getNodes(ipArray);
